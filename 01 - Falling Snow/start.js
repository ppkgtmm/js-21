(() => {
    function setup(){
        ...
        canvas.width = window.innerWidth // width = windows width
        canvas.height = window.innerHeight // height = windows height
        return {
            canvas,
    ...
    function drawSnowBalls(canvasContext,snowBall) {
        canvasContext.beginPath() // starting to draw
        canvasContext.arc(
            snowBall.x,
            snowBall.y,
            snowBall.radius,
            0,
            Math.PI*2) // drawing circle
        canvasContext.fillStyle = `rgba(255, 255, 255, ${snowBall.opacity})` // color,opacity
        canvasContext.fill()
